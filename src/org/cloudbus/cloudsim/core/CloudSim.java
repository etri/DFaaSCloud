	...
	public static double run() {
		...
		while (true) {
			if (runClockTick() || abruptTerminate) {
				System.out.println("CloudSim.run: break if (runClockTick() || abruptTerminate)");
				break;
			}
			//System.out.println("CloudSim.run: "+terminateAt+" "+clock);

			//System.out.println(clock+" "+terminateAt);
			// this block allows termination of simulation at a specific time
			if (terminateAt > 0.0 && clock >= terminateAt) {
				terminateSimulation();
				clock = terminateAt;
				System.out.println("CloudSim.run: break if (terminateAt > 0.0 && clock >= terminateAt) "+terminateAt+" "+clock);
				break;
			}
