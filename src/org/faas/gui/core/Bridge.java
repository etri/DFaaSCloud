...
import org.faas.utils.distribution.*;
...
public class Bridge {
			...
			try {
				...
				while(iter.hasNext()){
					...
					if(nodeType.equalsIgnoreCase("host")){  //host
						...
						Node fogDevice = new NodeGroupGui(nodeName, mips, ram, upBw, downBw, level, rate);
						...
						if(distType == Distribution.NORMAL){
							distribution = new NormalDistribution(0, new BigDecimal((Double)node.get("mean")).doubleValue(),
									new BigDecimal((Double)node.get("stdDev")).doubleValue());
						}
						else if(distType == Distribution.DETERMINISTIC) {
							distribution = new DeterministicDistribution(0, new BigDecimal((Double) node.get("value")).doubleValue());
						}
						else if(distType == Distribution.UNIFORM){
							distribution = new UniformDistribution(0, new BigDecimal((Double)node.get("mean")).doubleValue(),
									new BigDecimal((Double)node.get("max")).doubleValue());
						}
						else if(distType == Distribution.LOCATION){
							distribution = new LocationDistribution(0,
									new BigDecimal((Double)node.get("distCore")).doubleValue(),
									new BigDecimal((Double)node.get("distEdge")).doubleValue(),
									new BigDecimal((Double)node.get("distFog")).doubleValue(),
									new BigDecimal((Double)node.get("distCore")).doubleValue());
						}
						else if(distType == Distribution.TRACEBASEDTRAFFIC){
							distribution = new TraceBasedTrafficDistribution(0,
									(String)node.get("fileName"),
									new BigDecimal((Double)node.get("column")).intValue(),
									(String)node.get("seperator"));
						}
						else if(distType == Distribution.ZIPFWITHBASE){
							distribution = new ZipfWithBaseDistribution(0,
									new BigDecimal((Double)node.get("shape")).doubleValue(),
									new BigDecimal((Double)node.get("population")).intValue(),
									new BigDecimal((Double)node.get("baseQuantity")).doubleValue());
						}
						System.out.println("EventSource type : "+sensorType);
				...
				Logger.error("Bridge:jsonToGraph","Exception: " + e);
				// CWE-209
				//e.printStackTrace();
			}
	...
	public static String graphToJson(Graph graph){
		...
		for (Entry<Node, List<Edge>> entry : graph.getAdjacencyList().entrySet()) {
			...
			switch(srcNode.getType()){
				...
				case "SENSOR":
					...
					break;
				case "FOG_DEVICE":
					NodeGroupGui fogDevice = (NodeGroupGui)srcNode;
			...
			for (Edge edge : entry.getValue()) {
				...
				// CWE-569 (6) == -> equals()
				if(destNode.getType().equals("host") || destNode.getType().equals("core") || destNode.getType().equals("edge") ||
						destNode.getType().equals("FOG_DEVICE") || destNode.getType().equals("SENSOR") || destNode.getType().equals("ACTUATOR")){
					jobj2.put("latency", edge.getLatency());
				}else if(destNode.getName().equals("vm")){
		...
		try {
			...
			// CWE-209 add code
			Logger.error("Bridge:graphToJson","Exception: " + e);
			//e.printStackTrace();
		}
